...
#
from zhipuEmbeddingsData import get_file_paths_data
...
def chatbot_response(input, chatbot):
    """根据开关状态返回提示信息"""
...
def update_rag_data(data_list):
    get_file_paths_data(data_list)
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            update_rag_btn = gr.Button("更新rag数据库", variant="primary")

            status_output = gr.Textbox(label="运行状态", lines=5, max_lines=10)

        file = gr.Files(
            label="文件上传支持：md,txt,pdf,py,csv,json,docx,pptx,ppt,png,jpg,jpeg,url",
            file_types=["md","txt","pdf","py","csv","json","docx","pptx","ppt","png","jpg","jpeg","url"]
        )
    ...
    file.upload(fn=file_upload,inputs=file)
    ...
    submit_btn.click(
        fn=chatbot_response,
        inputs=[input, chatbot],  # 输入：用户消息 + 历史对话
